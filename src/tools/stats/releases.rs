...
pub async fn stats_releases(username: &str) {
    ...
    println_cli(format!("Total number of listens: {}", user_listens.get_listens().len()));
    ...
    for (key, data) in stats.into_sorted_vec(SortSorterBy::Count) {
        ...
        let artist_credit_string = release.get_or_fetch_artist_credits().await.unwrap().get_artist_credit_as_string();
        let pager_continue = pager.execute(|| {
            println!("[{}] - {} by {}", data.len(), release.title(), artist_credit_string);
        });
