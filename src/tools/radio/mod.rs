...
impl RadioCircle {
    async fn get_recording_of_artist(
        ...
        if self.unlistened {
            recordings = recordings
                .into_iter()
                .filter(|recording| !listens.has_recording(&recording.id) && !playlist.contains(recording))
                .collect_vec();
        }
    ...
    async fn get_recording_from_listen(&self, listen: &Listen, listens: &ListenCollection, playlist: &[Recording]) -> color_eyre::Result<Option<Recording>> {
    ...
    async fn create_list(&self, listens: &ListenCollection) -> color_eyre::Result<Vec<Recording>> {
        ...
        for listen in listen_shuffle.iter() {
            let result = self.get_recording_from_listen(listen.as_ref(), listens, &results).await?;
