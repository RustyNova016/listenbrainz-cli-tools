...
pub async fn clippy_tool() {
    ...
    let mut checked_ids = user_listens
        .get_mapped_listens()
    ...
    while let Some(current_id) = checked_ids.pop() {
        MUSICBRAINZ_DATABASE
            .remove(&current_id.clone().into_mbid())
            .await
            .unwrap();
        add_relations_to_queue(&mut checked_ids, &current_id)
            .await
            .unwrap();
...
async fn clippy_mbid(id: MBID) -> color_eyre::Result<bool> {
    ...
    if MissingWorkError::check_and_print(id).await? {
        error_count += 1
    }
